...
from rfdiffusion.util_module import create_custom_forward, init_lecun_normal, rbf
...
class TemplatePairStack(nn.Module):
    ...
    def __init__(self, n_block=2, d_templ=64, n_head=4, d_hidden=16, p_drop=0.25):
        ...
        proc_s = [
            PairStr2Pair(d_pair=d_templ, n_head=n_head, d_hidden=d_hidden, p_drop=p_drop)
...
class TemplateTorsionStack(nn.Module):
    def __init__(self, n_block=2, d_templ=64, n_head=4, d_hidden=16, p_drop=0.15):
        ...
        proc_s = [
            AttentionWithBias(d_in=d_templ, d_bias=d_templ, n_head=n_head, d_hidden=d_hidden)
            for i in range(n_block)
        ]
...
class Templ_emb(nn.Module):
    ...
    def __init__(
        ...
        self.templ_stack = TemplatePairStack(
            n_block=n_block, d_templ=d_templ, n_head=n_head, d_hidden=d_hidden, p_drop=p_drop
        )
...
class Recycling(nn.Module):
    ...
    def forward(self, seq, msa, pair, xyz, state):
        ...
        # warning: this can't be casted to float16 naively
        Cb = -0.58273431 * a + 0.56802827 * b - 0.54067466 * c + Ca

        dist = rbf(torch.cdist(Cb, Cb))
        dist = torch.cat((dist, left, right), dim=-1)

        # cast to the same type as the projection layer
        # might be float16 hopefully no loss of precision
        # dist = dist.to(self.proj_dist.weight.dtype)
